...
const title = formData.title
  ...
  return (
    <>
      ...
      >
       Upload image
        <input accept="image/*" name="image" onChange={handleInputChange} style={{ opacity: "0",position: 'absolute',height: '100%' }} type="file" />
      </Button>
          </Grid>
          <Grid item xs={12} sm={6}></Grid>
          <Grid item xs={12} sm={6}>
          {
            formData.imageUrl && <img className="uploadimage" src={formData.imageUrl} alt="upload"/>
          }
          {
                loading ? <Button type="submit" mt={2} disabled variant="contained" color="primary">
                Submit<CircularProgress />
              </Button> : <Button type="submit" mt={2} variant="contained" color="primary">
              Submit
            </Button>
              }
          </Grid>
